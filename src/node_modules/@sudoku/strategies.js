 ...
 * SudokuContext
 * 
 * Purpose: Acts as a state container and logic environment for Sudoku solving operations.
 * 
 * Key Responsibilities:
 * 1. State Management: Maintains the 9x9 grid and tracks potential candidate values (pencil marks) for every cell.
 * 2. Constraint Propagation: Automatically eliminates invalid candidates based on core Sudoku rules (Row, Column, and Box constraints).
 * 3. Change Detection: Provides an interface for strategies to update candidates and track whether the board state has evolved.
 */
class SudokuContext {
 ...
 * Implements a simulation-based scoring system to determine the optimal move.
 */
class RecommendationEngine {
    /**
     * Initializes the recommendation engine.
     * @param {StrategyFactory} factory - The factory to retrieve strategy instances.
     */
    constructor(factory) {
    ...
    /**
     * Counts the total number of candidates remaining across all empty cells.
     * @param {SudokuContext} context - The current simulation context.
     * @returns {number} The total count of candidates.
     * @private
     */
    ...
    /**
     * Calculates the number of solved cells in the grid.
     * A cell is considered solved if it is already filled or has exactly one candidate remaining.
     * @param {SudokuContext} context - The current simulation context.
     * @returns {number} The count of solved cells.
     * @private
     */
    _countSolvedCells(context) {
        let count = 0;
        for (let r = 0; r < SUDOKU_SIZE; r++) {
            for (let c = 0; c < SUDOKU_SIZE; c++) {
                if (context.grid[r][c] !== 0 || context.getCandidates(r, c).length === 1) {
                    count++;
                }
            }
        }
    ...
    /**
     * Analyzes the grid and returns the best strategy recommendation based on scoring.
     * @param {number[][]} grid - The current Sudoku grid state.
     * @returns {Object|null} The recommended strategy object or null if no strategy applies.
     */
    recommend(grid) {
        const candidates = [];
        
        const initialContext = new SudokuContext(grid);
        const initialSolved = this._countSolvedCells(initialContext);
        ...
        for (const profile of this.strategiesProfile) {
            ...
            if (isEffective) {
                const currentSolved = this._countSolvedCells(simContext);
                const currentCandidatesCount = this._countTotalCandidates(simContext);
                
                const solvedDiff = currentSolved - initialSolved;
                const removedDiff = initialCandidatesCount - currentCandidatesCount;

                // Scoring Formula:
                // 1. [Filled Cell]   Weight 100: Solving a cell strictly outweighs any amount of candidate elimination.
                // 2. [Elimination]   Weight 1:   Represents incremental progress when no cells can be solved directly.
                // 3. [Difficulty]    Cost < 1:   A tiny tie-breaker penalty to favor simpler strategies when results are equal  
                let score = (solvedDiff * 100) + (removedDiff * 1) - profile.difficultyCost;

                candidates.push({
                    strategy: profile.name,
                    reason: profile.reason,
                    details: { solved: solvedDiff, removed: removedDiff }, 
                    score: score,
                    profile: profile
                });
            }
        ...
        if (bestChoice.details.solved > 0) {
            dynamicReason += ` (Solves ${bestChoice.details.solved} cell(s) directly)`;

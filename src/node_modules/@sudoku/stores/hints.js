import { settings } from './settings';
import { writable } from 'svelte/store';
import { getCandidateSet, getRecommendedStrategy } from '@sudoku/strategies';

export const usedHints = writable(0);

/**
 * Factory function that creates and initializes the hint store.
 * @returns {Object} The hint store interface with subscribe, usage, and logic methods.
 */
function createHints() {
    const hints = writable(Infinity);
    let defaultHints = Infinity;

    settings.subscribe(($settings) => {
        if ($settings.hintsLimited) {
            defaultHints = $settings.hints;
            hints.update($hints => ($hints > $settings.hints ? $settings.hints : $hints));
        } else {
            defaultHints = Infinity;
            hints.set(Infinity);
        }
    });

    return {
        subscribe: hints.subscribe,

        useHint() {
            hints.update($hints => {
                if ($hints > 0) {
                    usedHints.update($useHints => $useHints + 1);
                    return $hints - 1;
                }
                return 0;
            });
        },

        /**
         * Orchestrates intelligent hint generation by analyzing the grid and updating the keyboard state.
         * @param {number[][]} grid - The current Sudoku board state.
         * @param {Object} cursor - Current selection coordinates {x, y}.
         * @param {Object} historyTree - The history manager instance for trial data.
         * @param {Function} updateKeyboard - Callback to update keyboard visual states.
         * @returns {string} Description text of the identified strategy or possible numbers.
         */
        handleSmartHint(grid, cursor, historyTree, updateKeyboard) {
            this.useHint();
            
            const recommendation = getRecommendedStrategy(grid);
            let strategyName = '';
            let finalCandidates = [];
            let hintText = '';

            if (recommendation?.strategy) {
                strategyName = recommendation.strategy;
                hintText = `${recommendation.strategy} -> ${recommendation.reason}`;
                const candidatesMap = getCandidateSet(grid, [strategyName]);
                finalCandidates = candidatesMap[cursor.y][cursor.x];
            } else {
                hintText = "Possible Numbers (Basic)";
                const candidatesBaseMap = getCandidateSet(grid, ['NakedSingle']);
                finalCandidates = candidatesBaseMap[cursor.y][cursor.x];
            }

            const triedNumbers = historyTree.getTriedForCell(cursor.x, cursor.y);
            
            updateKeyboard(finalCandidates || [], triedNumbers);

            return hintText;
        },

        reset() {
            hints.set(defaultHints);
            usedHints.set(0);
        }
    };
}

export const hints = createHints();
...
export class TreeHistoryManager {
  ...
  initGame(description = 'start game') {
    ...
    this.nodeCount = 1;
    this.printCurrentBoard('初始化游戏，当前棋盘状态:');
    this.printHistoryTree();
  ...
  saveState(description = '', backtrackable = false, infos = null) {
    ...
    this.isInBacktrackMode = false;
    this.printCurrentBoard('saveState 后，当前棋盘状态:');
    this.printHistoryTree();
  ...
  printCurrentBoard(msg = ' ') {
    ...
    const board = this.currentTreeNode.historyNode.state.digits;
    console.log(msg, this.currentTreeNode.id, '  当前棋盘状态:');
    ...
    console.log('当前节点信息:', {
      ...
      description: this.currentTreeNode.historyNode.action,
      backtrackable: this.currentTreeNode.backtrackable,
      infos: this.currentTreeNode.infos,
      parentId: this.currentTreeNode.parent?.id || '无',
      childrenCount: this.currentTreeNode.children?.length || 0,
    });
  }
  ...
  printNode(node, depth) {
    ...
    const isCurrent = node === this.currentTreeNode;
    const marker = isCurrent ? '[*]' : '[ ]';
    const parentId = node.parent?.id || null;

    const candidatesInfo = node.backtrackable ?
      ` Candidates: [${node.infos.candidates.join(', ')}], Tried: [${node.infos.tried.join(', ')}]` :
      '';
    
    console.log(`${indent}${marker} ${node.id}: ${node.historyNode.action} ${parentId} ${candidatesInfo}`);
...
getTriedForCell(x, y) {
    ...
    if (this.currentTreeNode &&
        this.currentTreeNode.backtrackable &&
        this.currentTreeNode.infos &&
        this.currentTreeNode.infos.cell.x === x &&
        this.currentTreeNode.infos.cell.y === y) {
        return this.currentTreeNode.infos.tried || [];
    }
    return [];
  }
}
...
export const historyTree = {
  ...
  canBacktrack: () => historyTreeManager.stats.canBacktrack,
   ...
   // --- 新增：暴露当前节点的详细信息 ---
  getCurrentNodeInfo: () => historyTreeManager.currentTreeNode,

  getTriedForCell: (x, y) => historyTreeManager.getTriedForCell(x, y)
};

...
export function decodeURL(url) {
    ...
    if (!match) throw new Error('Invalid SudokuWiki URL');
    const digits = match[1].replace(/\./g, '0');
    if (digits.length !== 81) throw new Error('Invalid Sudoku data length');
    const grid = [];
    for (let i = 0; i < 9; i++) {
...
export function validateURL(url) {
	return /sudokuwiki\.org\/sudoku\.htm\?bd=([0-9\.]{81})/.test(url);
}